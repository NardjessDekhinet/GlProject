...
function Offres() {
  ...
  const handleSubServiceClick = (subService) => {
    setSelectedSubService(
      selectedSubService === subService ? null : subService
    );
  };
        ...
        <div className="services-section">
          <div className="popular-services">
            ...
            {selectedWorkers.length === 0 && (
              <>
                <h2 className="text-xl mt-16 text-start font-bold mb-6">
                  {" "}
                  {t("Popular Services")}
                </h2>
                      ...
                      <button
                        className={`mt-4 px-4 py-2 rounded text-white ${
                          selectedWorkers.some(
                            (w) => w.workerName === worker.workerName
                          )
                ...
                <div className="w-full md:w-1/4 bg-white border border-gray-200  ">
                  {Object.keys(data).map((category) => (
                    <div key={category} className="border-b">
                      ...
                      {visibleCategory === category && (
                        <div>
                          {Object.keys(data[category].subServices).map(
                            (subService, index) => (
                              <button
                                key={index}
